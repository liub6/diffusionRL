...
@torch.no_grad()
def eval_actor(
        env: DMCGym, actor: nn.Module, device: str, n_episodes: int, seed: int, contexts: Optional[np.ndarray] = None, state_normalizer: Optional[StateNormalizer] = None
) -> np.ndarray:
...
def train(config: TrainConfig):
    ...
    state_dim = env.observation_space.shape[0] + 1 if config.context_aware else env.observation_space.shape[0]
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if t % config.eval_freq == 0 or t == config.max_timesteps - 1:
            ...
            log_dict = {"reward_mean": eval_score}
            wandb.log(log_dict, step=trainer.total_it)
