...
@dataclass
class Args:
    """if toggled, SAC will use diffusion generated data"""
    if_record: bool = True
    """if toggled, minari will record data"""
...
class ModifierWrapper(Wrapper):
    ...
    def step(self, action):
        ...
        #     info["final_info"] = {"episode" : {"r" : self.episode_reward}}
...
def make_env(env_id, seed, idx, capture_video, run_name, cond):
    def thunk():
        if capture_video and idx == 0:
            # env = gym.make(env_id, render_mode="rgb_array", length = Args().length)
            env = DMCGym("cartpole", "swingup", task_kwargs={'random':seed})
            env = ModifierWrapper(env, cond)
            env = gym.wrappers.RecordVideo(env, f"videos/{run_name}")
        else:
            # env = gym.make(env_id, length = Args().length)
            env = DMCGym("cartpole", "swingup", task_kwargs={'random':seed})
            env = ModifierWrapper(env, cond)
        env = DataCollector(env, record_infos=True, max_buffer_steps=100000, observation_space=env.observation_space, action_space=env.action_space)
...
"""
    ...
    args = tyro.cli(Args)
    run_name = f"{args.env_id}_length{args.length}_seed{args.seed}"
    if args.track:
        import wandb
    ...
    # env setup
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, args.seed, 0, args.capture_video, run_name, args.cond) for i in range(args.num_envs)], 
        )
    ...
    if Args().if_record:
        for i, env in enumerate(envs.envs):
            ...
            author="BoxuLiu",
            author_email="boxu.liu@aalto.fi"
            )
